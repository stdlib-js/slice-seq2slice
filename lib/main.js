...
function seq2slice( str, len, strict ) {
	...
	var s;
	if ( !isString( str ) ) {
		throw new TypeError( format( '1j5Ej', str ) );
	}
	if ( !isNonNegativeInteger( len ) ) {
		throw new TypeError( format( '1j53X', len ) );
	}
	if ( !isBoolean( strict ) ) {
		throw new TypeError( format( '1j5AL', strict ) );
	}
	...
	if ( code === 'ERR_SLICE_INVALID_INCREMENT' ) {
		throw new RangeError( format( '1j5Ek', str ) );
	}
	if ( code === 'ERR_SLICE_OUT_OF_BOUNDS' ) {
		throw new RangeError( format( '1j5El', str ) );
	}
